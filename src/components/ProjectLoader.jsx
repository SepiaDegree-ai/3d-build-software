...
export default function ProjectLoader({ onProjectLoad }) {
  ...
  const fetchProjects = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/projects/${currentUser.uid}`);
  ...
  const handleProjectLoad = async (projectId) => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/projects/project/${projectId}`);
